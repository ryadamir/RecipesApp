...
/**
 * This is an object used for checking internet status in the main activity
 */
...
object Network {
    ...
    private fun connectivityStatusString(context: Context): Int {
        ...
        var status = -1
        if (connection == TYPE_WIFI) status =
            NETWORK_STATUS_WIFI else if (connection == TYPE_MOBILE) status =
            NETWORK_STATUS_MOBILE else if (connection == TYPE_NOT_CONNECTED) status =
